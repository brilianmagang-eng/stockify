...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">record new transactions</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">Note new stock movements (in or out).</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                        ...
                        <option value="in" {{ old('type') == 'in' ? 'selected' : '' }}>Incoming Stock (from Suppliers, Returns, etc.)</option>
                        <option value="out" {{ old('type') == 'out' ? 'selected' : '' }}>Outgoing Stock (Sales, Damaged, etc.)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="quantity" name="quantity" value="{{ old('quantity') }}" min="1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required placeholder="Example: 50">
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="notes" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Note (Optional)</label>
                    <textarea id="notes" name="notes" rows="3" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Example: Receipt from Supplier ABC">{{ old('notes') }}</textarea>
                </div>
            </div>
