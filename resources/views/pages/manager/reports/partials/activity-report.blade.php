<h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Laporan Aktivitas Pengguna</h3>
<form method="GET" action="{{ route('admin.reports.index') }}" class="mb-6">
    <input type="hidden" name="tab" value="activity">
     <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-white dark:bg-gray-900 rounded-lg">
        <div>
            <label class="text-sm">Pengguna</label>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">@if($activity->type == 'in') Mencatat barang masuk @else Mencatat barang keluar @endif</td>
                <td class="px-6 py-4">{{ $activity->quantity }} unit "{{ $activity->product->name ?? 'N/A' }}"</td>
            </tr>
            @empty
            <tr><td colspan="4" class="text-center p-4">Tidak ada aktivitas.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>