<h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Transactions Report</h3>

<form method="GET" action="{{ route('manager.reports.index') }}" class="mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-white dark:bg-gray-900 rounded-lg">
        <div>
            ...
            <input type="date" name="start_date" value="{{ request('start_date') }}" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600">
        </div>
        <div>
            <label for="end_date" class="text-sm">End Date</label>
            <input type="date" name="end_date" value="{{ request('end_date') }}" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600">
        </div>
        <div>
            ...
            <select name="type" class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600">
                ...
                <option value="in" @selected(request('type') == 'in')>Incoming</option>
                <option value="out" @selected(request('type') == 'out')>Outgoing</option>
